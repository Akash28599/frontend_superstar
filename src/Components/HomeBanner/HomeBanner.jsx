...
const HomeBanner = () => {
  ...
  const { homeBanner, cocoHead, homeBannerLoading: loading } = useHomeStore();

  if (loading) return (
    <div className="min-h-screen bg-kelloggs-red text-white flex items-center justify-center text-[1.2rem]">
      Loading...
    </div>
  );

  const items = Array.isArray(homeBanner) ? homeBanner : [];
  const banner = items.find(i => i.uid === 'homebanner');
  const cloudItem = items.find(i => i.uid === 'cloud');
  ...
  const cloudImage = cloudItem?.image && (
    cloudItem.image.formats?.medium?.url ||
    cloudItem.image.formats?.small?.url ||
    cloudItem.image.formats?.thumbnail?.url ||
    cloudItem.image.url
  );

  // Compute star image
  const starItem = Array.isArray(cocoHead) ? cocoHead.find(d => !(d.icon_description && d.icons)) : null;
  const starImage = starItem?.icons && (
  ...
  // Banner images
  const logoImage = banner.nav_icon?.formats?.small?.url ||
    banner.nav_icon?.formats?.thumbnail?.url ||
    banner.nav_icon?.url;

  const heroImage = banner.image?.formats?.large?.url ||
    banner.image?.formats?.medium?.url ||
    banner.image?.url;

  const Cloud = ({ className }) => cloudImage && (
    <img src={cloudImage} alt="Cloud" className={`absolute pointer-events-none z-[3] ${className}`} />
  );
    return (
        ...
        `}>
            ...
            <button
              className={`
                    ...
                    bg-white scale-100 text-kelloggs-red
                      hover:bg-kelloggs-gold hover:scale-105
